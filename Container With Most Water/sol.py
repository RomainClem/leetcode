from pip import List, main


class Solution:
    def maxArea(self, height: List[int]) -> int:
        max_area: int = min(height[0], height[1]) * 1
        left_vline: List[int,int] = [0, height[0]]
        right_vline: List[int,int] = [1, height[1]]
        discovered: dict[int: int]= {
            height[0]: 0,
            height[1]: 1
        }
        
        if len(height) == 2: return max_area
        
        for i in range(2, len(height)):
            
            vol_l: int = min(height[i], left_vline[1]) * (i-left_vline[0])
            vol_r: int = min(height[i], right_vline[1]) * (i-right_vline[0])            
